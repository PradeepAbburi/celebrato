...
export const AuthPage: React.FC<AuthPageProps> = ({
  ...
  const { currentUser, profile, signInWithGoogle, signInWithEmail, signUpWithEmail, verifyAdminCode, signOut } = useAuth();
  const [authMode, setAuthMode] = useState<'login' | 'signup' | 'admin'>(defaultMode);
  ...
  const handleAdminSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    if (currentUser) {
      setError('Active user account detected. You must log out of your user account before logging in as Administrator.');
      return;
    }
    if (!adminPasscode.trim()) {
      setError('Please enter your administrator security code.');
      return;
    }
    setLoading(true);
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'Access Denied: Invalid administrator code.');
      }
    } catch (err: any) {
      ...
      <div className="hidden lg:block lg:w-1/2 xl:w-[55%] relative overflow-hidden">
        ...
        <div className="absolute bottom-10 left-8 right-8 space-y-3">
          ...
          <p className="text-sm text-gray-300 max-w-md leading-relaxed">
            {authMode === 'admin'
              ? 'Secured operations gateway for venue managers, reservations desk, and financial reporting.'
              : authMode === 'signup'
          ...
          <div className="flex flex-col gap-2 pt-2">
            {[
              ...
              { icon: <ShieldCheck className="w-4 h-4" />, color: 'bg-emerald-500/20 text-emerald-400', title: 'Admin & Host Separation', sub: 'Strict security boundaries with isolated administration' },
            ].map((perk, i) => (
        ...
        <div className="max-w-sm w-full mx-auto space-y-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-400">
              {authMode === 'admin'
                ? 'Enter your confidential administrator passcode to unlock the console.'
                : authMode === 'signup'
              ...
              {currentUser ? (
                ...
                <form onSubmit={handleAdminSubmit} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-[#242424] border border-[#383838] rounded-2xl text-sm font-mono tracking-wider text-white placeholder-gray-500 focus:outline-none focus:border-amber-500 transition"
                      />
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-black text-sm rounded-2xl transition shadow-lg shadow-amber-500/20 active:scale-[0.99] disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
                  >
                    {loading ? 'Verifying Code...' : (
                      <>Verify Code & Unlock Console <ArrowRight className="w-4 h-4" /></>
                    )}
                  </button>
                </form>
              )}

              {/* Back to regular host login */}
              <div className="flex items-center justify-center pt-2">
                <button
                  type="button"
                  id="auth-back-to-host-login-btn"
                  onClick={() => {
                    setAuthMode('login');
                    setError('');
                  }}
                  className="text-xs text-gray-400 hover:text-white font-semibold transition cursor-pointer flex items-center gap-1.5"
                >
                  <ArrowLeft className="w-3.5 h-3.5 text-amber-400" />
                  <span>Return to Host Sign In</span>
                </button>
              </div>
            </div>
          ) : (
            /* ──────── MODE: STANDARD USER LOGIN / SIGNUP ──────── */
            <div className="space-y-4">
              {/* Google Sign In */}
              <button
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-4">
                ...
                >
                  {loading ? 'Processing...' : authMode === 'signup' ? (
              ...
              {/* ── ADMIN LOGIN BUTTON IN AUTH PAGE ── */}
              <div className="pt-4 border-t border-[#2d2d2d] space-y-2">
                ...
                <button
                  id="auth-admin-login-entry-btn"
                  type="button"
                  onClick={() => {
                    setAuthMode('admin');
                    setError('');
                  }}
                  className="w-full py-2.5 px-4 rounded-xl bg-[#242424] hover:bg-[#2c2c2c] border border-[#383838] hover:border-amber-500/50 text-xs font-bold text-gray-300 hover:text-amber-400 transition flex items-center justify-center gap-2 cursor-pointer active:scale-98 shadow-sm"
                >
